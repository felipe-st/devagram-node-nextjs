    ...
    try{
        if(req.method === 'PUT'){
            ...
            if(!usuarioASerSeguido){
                return res.status(400).json({erro: 'Usuario a ser seguido nao localizado'})
            }
            ...
            if(euJaSigoEsseUsuario && euJaSigoEsseUsuario.length > 0){
                ...
                // o usuario logado esta seguindo um usario novo
                // o numero de seguindo dele tem que aumentar
                // adicionar um seguindo no usuario logado
                usuarioLogado.seguindo++;
                await UsuarioModel.findByIdAndUpdate({_id: usuarioLogado._id}, usuarioLogado);

                // o usuario logado esta sendo seguido por um novo seguidor
        ...
        console.log(e);
        return res.status(500).json({erro: 'Nao foi possivel seguir/(des)seguir o usuario informado'});
    }
}
